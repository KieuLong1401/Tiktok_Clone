import styles from './Sidebar.module.scss'

import Icon from '../../../components/Icon'
import Button from '../../../components/Button/Button'

import config from '../../../configs'

const routes = config.routes

function Sidebar() {
    return (
        <div className={styles.wrapper}>
            <div className={styles.menu}>
                <Button
                    leftIcon={
                        <Icon
                            child={
                                <svg
                                    width='32'
                                    data-e2e=''
                                    height='32'
                                    viewBox='0 0 48 48'
                                    fill='rgba(22, 24, 35, 1)'
                                    xmlns='http://www.w3.org/2000/svg'>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M23.0484 7.84003C23.6014 7.38666 24.3975 7.38666 24.9504 7.84001L41.051 21.04C41.5411 21.4418 41.7258 22.1082 41.5125 22.705C41.2991 23.3017 40.7338 23.7 40.1 23.7H37.769L36.5769 36.7278C36.4592 38.0149 35.3798 39 34.0873 39H13.9127C12.6202 39 11.5409 38.0149 11.4231 36.7278L10.231 23.7H7.89943C7.2657 23.7 6.70035 23.3017 6.487 22.705C6.27364 22.1083 6.45833 21.4418 6.9484 21.04L23.0484 7.84003ZM23.9995 10.9397L12.0948 20.7H12.969L14.369 36H22.4994V28.3138C22.4994 27.7616 22.9471 27.3138 23.4994 27.3138H24.4994C25.0517 27.3138 25.4994 27.7616 25.4994 28.3138V36H33.631L35.031 20.7H35.9045L23.9995 10.9397Z'></path>
                                </svg>
                            }
                            triggered={
                                <svg
                                    width='32'
                                    data-e2e=''
                                    height='32'
                                    viewBox='0 0 48 48'
                                    fill='rgba(254, 44, 85, 1)'
                                    xmlns='http://www.w3.org/2000/svg'>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M24.9505 7.84001C24.3975 7.38666 23.6014 7.38666 23.0485 7.84003L6.94846 21.04C6.45839 21.4418 6.2737 22.1083 6.48706 22.705C6.70041 23.3017 7.26576 23.7 7.89949 23.7H10.2311L11.4232 36.7278C11.5409 38.0149 12.6203 39 13.9128 39H21.5C22.0523 39 22.5 38.5523 22.5 38V28.3153C22.5 27.763 22.9477 27.3153 23.5 27.3153H24.5C25.0523 27.3153 25.5 27.763 25.5 28.3153V38C25.5 38.5523 25.9477 39 26.5 39H34.0874C35.3798 39 36.4592 38.0149 36.577 36.7278L37.7691 23.7H40.1001C40.7338 23.7 41.2992 23.3017 41.5125 22.705C41.7259 22.1082 41.5412 21.4418 41.0511 21.04L24.9505 7.84001Z'></path>
                                </svg>
                            }
                            trigAt={routes.root}
                            link={false}
                        />
                    }
                    className={styles.menuElement}
                    to={routes.root}
                    activeTo>
                    For You
                </Button>
                <Button
                    leftIcon={
                        <Icon
                            child={
                                <svg
                                    width='32'
                                    data-e2e=''
                                    height='32'
                                    viewBox='0 0 48 48'
                                    fill='rgba(22, 24, 35, 1)'
                                    xmlns='http://www.w3.org/2000/svg'>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M18 12.5C15.5897 12.5 13.5849 14.5018 13.5849 17.0345C13.5849 19.5672 15.5897 21.569 18 21.569C20.4103 21.569 22.4151 19.5672 22.4151 17.0345C22.4151 14.5018 20.4103 12.5 18 12.5ZM10.5849 17.0345C10.5849 12.9017 13.8766 9.5 18 9.5C22.1234 9.5 25.4151 12.9017 25.4151 17.0345C25.4151 21.1673 22.1234 24.569 18 24.569C13.8766 24.569 10.5849 21.1673 10.5849 17.0345ZM18 29.8793C14.0801 29.8793 10.7403 32.5616 9.69697 36.2673C9.5473 36.7989 9.03833 37.1708 8.49337 37.0811L7.50662 36.9189C6.96166 36.8292 6.58837 36.3131 6.72325 35.7776C8.00732 30.6788 12.5509 26.8793 18 26.8793C23.449 26.8793 27.9927 30.6788 29.2767 35.7776C29.4116 36.3131 29.0383 36.8292 28.4934 36.9189L27.5066 37.0811C26.9617 37.1708 26.4527 36.7989 26.303 36.2673C25.2597 32.5616 21.9199 29.8793 18 29.8793Z'></path>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M33 31.5371C32.2445 31.5371 31.5198 31.668 30.8447 31.9093C30.3246 32.0951 29.7189 31.9243 29.4549 31.4392L28.9769 30.5608C28.713 30.0757 28.8907 29.463 29.4009 29.2516C30.513 28.791 31.7285 28.5371 33 28.5371C37.4554 28.5371 41.1594 31.6303 42.2706 35.7812C42.4135 36.3147 42.0386 36.8308 41.4935 36.9196L40.5065 37.0804C39.9614 37.1692 39.4546 36.7956 39.2894 36.2686C38.4217 33.5 35.91 31.5371 33 31.5371Z'></path>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M33 18.5C31.6193 18.5 30.5 19.6193 30.5 21C30.5 22.3807 31.6193 23.5 33 23.5C34.3807 23.5 35.5 22.3807 35.5 21C35.5 19.6193 34.3807 18.5 33 18.5ZM27.5 21C27.5 17.9624 29.9624 15.5 33 15.5C36.0376 15.5 38.5 17.9624 38.5 21C38.5 24.0376 36.0376 26.5 33 26.5C29.9624 26.5 27.5 24.0376 27.5 21Z'></path>
                                </svg>
                            }
                            triggered={
                                <svg
                                    width='32'
                                    data-e2e=''
                                    height='32'
                                    viewBox='0 0 48 48'
                                    fill='rgba(254, 44, 85, 1)'
                                    xmlns='http://www.w3.org/2000/svg'>
                                    <path d='M25.5 17C25.5 21.1421 22.1421 24.5 18 24.5C13.8579 24.5 10.5 21.1421 10.5 17C10.5 12.8579 13.8579 9.5 18 9.5C22.1421 9.5 25.5 12.8579 25.5 17Z'></path>
                                    <path d='M7.10396 34.7906C8.78769 30.2189 12.8204 27 18.0009 27C23.1818 27 27.2107 30.2213 28.8958 34.7898C29.3075 35.906 28.6141 37 27.5 37H8.5C7.38629 37 6.69289 35.9067 7.10396 34.7906Z'></path>
                                    <path d='M40.6308 37H32C31.2264 34.1633 30.0098 31.5927 28.144 29.7682C29.5384 28.9406 31.1829 28.5 33 28.5C37.239 28.5 40.536 30.8992 41.9148 35.0108C42.2516 36.0154 41.5423 37 40.6308 37Z'></path>
                                    <path d='M33 26.5C36.0376 26.5 38.5 24.0376 38.5 21C38.5 17.9624 36.0376 15.5 33 15.5C29.9624 15.5 27.5 17.9624 27.5 21C27.5 24.0376 29.9624 26.5 33 26.5Z'></path>
                                </svg>
                            }
                            trigAt={routes.following}
                            link={false}
                        />
                    }
                    className={styles.menuElement}
                    to={routes.following}
                    activeTo>
                    Following
                </Button>
                <Button
                    leftIcon={
                        <Icon
                            child={
                                <svg
                                    fill='rgba(22, 24, 35, 1)'
                                    width='1em'
                                    height='1em'
                                    viewBox='0 0 48 48'
                                    xmlns='http://www.w3.org/2000/svg'>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M24 37.4a13.4 13.4 0 1 0 0-26.8 13.4 13.4 0 0 0 0 26.8ZM40.5 24a16.5 16.5 0 1 1-33 0 16.5 16.5 0 0 1 33 0Z'></path>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M27.13 27.18a2 2 0 0 0 .93-1.32l1.84-9.33a.6.6 0 0 0-.9-.63l-8.14 4.92a2 2 0 0 0-.92 1.32l-1.84 9.33c-.1.52.45.9.9.63l8.13-4.92Zm-5.04-.45 3.11-1.89.7-3.57-3.1 1.89-.7 3.57Z'></path>
                                </svg>
                            }
                            triggered={
                                <svg
                                    fill='rgba(254, 44, 85, 1)'
                                    width='1em'
                                    height='1em'
                                    viewBox='0 0 48 48'
                                    xmlns='http://www.w3.org/2000/svg'>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M24 40.5a16.5 16.5 0 1 0 0-33 16.5 16.5 0 0 0 0 33Zm4.43-14.54c-.12.6-.49 1.12-1.01 1.44l-8.88 5.37a.65.65 0 0 1-.98-.69l2.01-10.18c.12-.6.49-1.12 1.01-1.44l8.88-5.37a.65.65 0 0 1 .98.69l-2.01 10.18Z'></path>
                                    <path d='m21.92 26.89 3.4-2.05.76-3.9-3.4 2.06-.76 3.89Z'></path>
                                </svg>
                            }
                            trigAt={routes.explore}
                            link={false}
                        />
                    }
                    className={styles.menuElement}
                    to={routes.explore}
                    activeTo>
                    Explore
                </Button>
                <Button
                    leftIcon={
                        <Icon
                            child={
                                <svg
                                    width='32'
                                    data-e2e=''
                                    height='32'
                                    viewBox='0 0 32 32'
                                    fill='rgba(22, 24, 35, 1)'
                                    xmlns='http://www.w3.org/2000/svg'>
                                    <path
                                        fillRule='evenodd'
                                        clipRule='evenodd'
                                        d='M7.78511 10.3334C6.95518 10.3334 6.33301 10.9792 6.33301 11.7143V20.2858C6.33301 21.0209 6.95518 21.6667 7.78511 21.6667H18.5744C19.4043 21.6667 20.0265 21.0209 20.0265 20.2858V17.5602C20.0265 17.1826 20.2392 16.8372 20.5763 16.6672C20.9135 16.4973 21.3177 16.5317 21.6212 16.7563L25.6663 19.7488V12.2513L21.6212 15.2439C21.3177 15.4684 20.9135 15.5029 20.5763 15.3329C20.2392 15.1629 20.0265 14.8175 20.0265 14.4399V11.7143C20.0265 10.9792 19.4043 10.3334 18.5744 10.3334H7.78511ZM25.6855 12.2371C25.6831 12.2388 25.6839 12.2383 25.6839 12.2383L25.6855 12.2371ZM25.6716 12.2177C25.673 12.2212 25.6746 12.2243 25.6763 12.2269C25.6798 12.2324 25.6834 12.2355 25.6855 12.2371L25.6874 12.2383C25.6874 12.2383 25.6865 12.238 25.6839 12.2383M4.33301 11.7143C4.33301 9.81952 5.90653 8.33337 7.78511 8.33337H18.5744C20.453 8.33337 22.0265 9.81953 22.0265 11.7143V12.4562L24.4963 10.629C25.0929 10.1877 25.8879 10.1155 26.5542 10.4359C27.224 10.758 27.6663 11.4325 27.6663 12.1905V19.8096C27.6663 20.5676 27.224 21.2421 26.5542 21.5642C25.888 21.8846 25.0929 21.8124 24.4963 21.371L22.0265 19.5439V20.2858C22.0265 22.1806 20.453 23.6667 18.5744 23.6667H7.78511C5.90653 23.6667 4.33301 22.1806 4.33301 20.2858V11.7143Z'></path>
                                    <path d='M15 15.134C15.6667 15.5189 15.6667 16.4811 15 16.866L12 18.5981C11.3333 18.983 10.5 18.5019 10.5 17.7321L10.5 14.2679C10.5 13.4981 11.3333 13.017 12 13.4019L15 15.134Z'></path>
                                </svg>
                            }
                            triggered={
                                <svg
                                    width='32'
                                    data-e2e=''
                                    height='32'
                                    viewBox='0 0 48 48'
                                    fill='rgba(254, 44, 85, 1)'
                                    xmlns='http://www.w3.org/2000/svg'>
                                    <path d='M6.5 17.5714C6.5 14.7292 8.86029 12.5 11.6782 12.5H27.8621C30.6799 12.5 33.0402 14.7292 33.0402 17.5714V18.6843L36.745 15.9435C37.6399 15.2815 38.8324 15.1731 39.8318 15.6537C40.8365 16.1369 41.5 17.1486 41.5 18.2857V29.7143C41.5 30.8514 40.8365 31.8631 39.8318 32.3463C38.8324 32.8269 37.6399 32.7185 36.745 32.0565L33.0402 29.3158V30.4286C33.0402 33.2708 30.6799 35.5 27.8621 35.5H11.6782C8.86029 35.5 6.5 33.2708 6.5 30.4286V17.5714Z'></path>
                                    <path
                                        d='M23.25 23.134C23.9167 23.5189 23.9167 24.4811 23.25 24.866L17.25 28.3301C16.5833 28.715 15.75 28.2339 15.75 27.4641L15.75 20.5359C15.75 19.7661 16.5833 19.285 17.25 19.6699L23.25 23.134Z'
                                        fill='white'></path>
                                </svg>
                            }
                            trigAt={routes.live}
                            link={false}
                        />
                    }
                    className={styles.menuElement}
                    to={routes.live}
                    activeTo>
                    LIVE
                </Button>
            </div>
            <div className={styles.followingAccount}>
                <h2>Following accounts</h2>
                <p>Accounts you follow will appear here</p>
            </div>
            <div className={styles.space}></div>
            <footer>
                <div className={styles.createEffectBtnWrapper}>
                    <a
                        href='https://effecthouse.tiktok.com/download?utm_campaign=ttweb_entrance_v2&utm_source=tiktok_webapp_main'
                        className={styles.createEffectBtn}>
                        <Button
                            leftIcon={
                                <svg xmlns='http://www.w3.org/2000/svg' width='1em' height='1em'>
                                    <path
                                        fill='black'
                                        d='M0 18V9a9 9 0 0 1 18 0v9H0Zm15.75-2.25V9a6.75 6.75 0 0 0-9.978-5.93 6.744 6.744 0 0 1 6.619 2.971A4.483 4.483 0 0 1 13.5 9v6.75h2.25ZM6.502 5.257A4.5 4.5 0 0 0 2.25 9.75v6H4.5V9c0-1.56.795-2.936 2.002-3.743Zm4.096 2.16A2.25 2.25 0 0 0 6.75 9v6.75h4.5v-6a4.48 4.48 0 0 0-.652-2.333Z'></path>
                                </svg>
                            }
                            className={styles.createEffectBtnContent}>
                            Create effects
                        </Button>
                        <img
                            src='https://sf16-website-login.neutral.ttwstatic.com/obj/tiktok_web_login_static/tiktok/webapp/main/webapp-desktop/045b2fc7c278b9a30dd0.png'
                            alt='create effects button background image'
                        />
                    </a>
                </div>
                <div className={styles.linkContainer}>
                    <div className={styles.linkBox}>
                        <a href={routes.about}>About</a>
                        <a href='http://newsroom.localhost:5173'>Newsroom</a>
                        <a href={routes.contact}>Contact</a>
                        <a href='http://careers.localhost:5173'>Careers</a>
                    </div>
                    <div className={styles.linkBox}>
                        <a href={routes.forGood}>TikTok for Good</a>
                        <a href={routes.business}>Advertise</a>
                        <a href='http://developers.localhost:5173'>Developers</a>
                        <a href={routes.transparency}>Transparency</a>
                        <a href={routes.tiktokReward}>TikTok Rewards</a>
                        <a href={routes.embed}>TikTok Embeds</a>
                    </div>
                    <div className={styles.linkBox}>
                        <a href='http://support.localhost:5173'>Help</a>
                        <a href={routes.safety}>Safety</a>
                        <a href={routes.temps}>Terms</a>
                        <a href={routes.privacy}>Privacy</a>
                        <a href={routes.creatorPortal}>Creator Portal</a>
                        <a href={routes.community}>Community Guidelines</a>
                    </div>
                    <span className={styles.copyRight}>© 2023 TikTok</span>
                </div>
            </footer>
        </div>
    )
}

export default Sidebar
